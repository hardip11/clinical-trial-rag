...
Structure:
    run_pipeline(llm, embeddings) → PipelineResult
        Phase 1: Ingestion (PDF loading, chunking, vectorstore)
        Phase 2: Structured extraction (eligibility criteria → JSON)
        Phase 3: Multi-agent audit (deterministic + LLM second-pass)
        Phase 3.5: Ambiguity detection (regulatory risk flagging)
        Phase 4: RAGAS-style evaluation (faithfulness + context precision)

    interactive_qa(llm, retriever) → session_log
        Interactive Q&A loop with confidence scoring and source tracing.
"""
...
def build_models():
    """
    Initialize embedding model and LLM endpoint.

    Kept separate from run_pipeline() so models can be reused across
    multiple pipeline runs (e.g., in tests or batch processing) without
...
def run_pipeline(llm, embeddings) -> dict:
    """
    Execute all 4 pipeline phases and return a summary dict.

    Phases:
        1. Ingestion   — PDF loading, chunking, vectorstore (cached)
        2. Extraction  — Structured eligibility criteria → JSON (cached)
        3. Audit       — Deterministic rulebook + LLM second-pass
        3.5 Ambiguity  — Regulatory risk flagging
        4. Evaluation  — RAGAS-style faithfulness + context precision

    Args:
        llm: ChatHuggingFace instance.
        embeddings: HuggingFaceEmbeddings instance.

    Returns:
        Dict with keys: retriever, extracted_data, audit_report,
        eval_results, timestamp.
    """
    if not Path(PDF_PATH).exists():
        print(f"❌ Protocol PDF not found at '{PDF_PATH}'.")
        print("   Place your PDF at data/Protocol.pdf and retry.")
        sys.exit(1)
    ...
    if cache_hit:
        ...
        if clean_json_str:
            ...
            retry_prompt_text = RETRY_EXTRACTION_PROMPT.format(
                context=markdown_context[:3000]
            )
            raw_retry = llm.invoke(retry_prompt_text).content
    ...
    for record in extracted_data:
        print(f"  {record.get('age_group'):6} | {record.get('metric'):30} | "
              f"Score ≥ {record.get('minimum_score')} | §{record.get('section')}")
    ...
    print("\nRunning LLM second-pass audit (Agent 3)...")
    ...
    broad_docs = retriever.retrieve(
        "patient eligibility criteria inclusion exclusion", verbose=False
    )
    ...
    # Golden dataset is specific to the CAMPFIRE protocol (pages 10–20).
    # Update gold_pages and faithfulness_terms if you switch protocols.
    golden_dataset = [
        {
    ...
    print(f"  Note: Faithfulness uses keyword-term matching as a hallucination")
    print(f"        proxy. For production use, replace with an LLM-as-judge call.")
    ...
    with open(eval_path, "w") as f:
        json.dump({
            "timestamp": timestamp,
            "scoring_note": (
                "Faithfulness uses keyword-term matching as a hallucination proxy. "
                "For production, replace with an LLM-as-judge entailment prompt."
            ),
...
def interactive_qa(llm, retriever, timestamp: str) -> list[dict]:
    """
    Run an interactive Q&A session over the ingested protocol.

    Each answer is confidence-scored (0–100) and traced to source pages.
    Falls back to plain QA if the structured JSON response cannot be parsed.
    Session is saved to data/sample_output/session_<timestamp>.json on exit.

    Args:
        llm: ChatHuggingFace instance.
        retriever: Initialized HybridRetriever.
        timestamp: Timestamp string used for the output filename.

    Returns:
        List of session log dicts (query, answer, confidence).
    """
    print("\n" + "=" * 60)
    ...
    while True:
        ...
        try:
            ...
            # Fallback to plain QA if structured parsing failed
            if answer == "N/A":
...
if __name__ == "__main__":
    ...
    result = run_pipeline(llm, embeddings)
    interactive_qa(llm, result["retriever"], result["timestamp"])
